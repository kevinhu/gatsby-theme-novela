...
```sh
gatsby new blog-theme https://github.com/kevinhu/blog-theme
```

##### With `git clone`:

```sh
git clone git@github.com:kevinhu/blog-theme.git novela-site
...
### Step 3: Using `blog-theme`

You must add `blog-theme` as a plugin in `gatsby-config.js`.
`blog-theme` is configurable through plugin options but we will use the defaults for now.
...
Once you've setup `blog-theme` plugin in `gatsby-config.js` you can start creating your first Posts. In order to create a Post you also need at least one Author.
...
Once you have your project setup with `gatsby-source-contentful` and `blog-theme` and your Contentful space is setup with the imported model you are good to go.
...
[View Author example](https://github.com/kevinhu/blog-theme/blob/master/content/authors/authors.yml)
...
| social   | required |  Array  | A list of social accounts and urls. [View supported icon/name combinations](https://github.com/kevinhu/blog-theme/blob/master/%40narative/blog-theme/src/components/SocialLinks/SocialLinks.tsx#L15) |
...
| social        | required |   Array    | [View supported icon/name combinations](https://github.com/kevinhu/blog-theme/blob/master/%40narative/gatsby-theme-novela/src/components/SocialLinks/SocialLinks.tsx#L15). `url` is required and the icon is chosen based on the url. If you also provide `name`, then the icon will be the same as the name you provided as long as the name equals one of the supported icons (See example bellow). Used in site footer and meta tags |
